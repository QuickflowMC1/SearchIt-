...
The GN args used to build SearchIt! are available here: [SearchIt!.gn_args](../build/SearchIt!.gn_args).
The patches are to be applied second the order specified in the `SearchIt!_patches_list.txt` file (you can use `git am`).
...
`uazo/SearchIt!-build:(VERSION)-(COMMIT)`

You can see all the images released in https://hub.docker.com/r/uazo/SearchIt!-build/tags
...
CHVERSION=uazo/SearchIt!-build:$VERSION-$SHA
CONTAINER=dev1
BDEBUG=false
```
...
You can use the [script](https://github.com/uazo/SearchIt!/blob/master/tools/images/win-sdk/prepare.sh) to generate it. 

all release builds are done via the [action available](https://github.com/uazo/SearchIt!/blob/master/.github/workflows/build_SearchIt!.yaml) from which you can see the mode I have adopted.
