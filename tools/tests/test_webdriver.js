...
async function testApp(isLocal, config) {
	...
	let selectorKey = "org.SearchIt!.SearchIt!";
	...
	try {
		...
		// enable SearchIt! test pref
		await clickByUIautomator(
			'new UiSelector().text("Enable support for SearchIt! test")');

		// enable webview mode (only if chromium or is local)
		if (!isLocal && !isChromium) {
			await clickByUIautomator(
				'new UiSelector().text("Enable webview support for SearchIt! test")');
		}
		...
		// start update of SearchIt! fonts pack
		await driver.executeScript(
			'await window.SearchIt!.startComponentUpdate("gcmjkmgdlgnkkcocmoeiminaijmmjnii")', []);

		count = 0;
		for (; ;) {
			let version = await driver.executeScript(
				'return JSON.parse(await window.SearchIt!.getComponentData("gcmjkmgdlgnkkcocmoeiminaijmmjnii")).version;', []);
			console.log(version);
